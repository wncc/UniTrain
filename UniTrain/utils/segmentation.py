...
def get_data_loader(
    data_dir: str, batch_size: int, shuffle: bool = True, transform=None, split="train"
) -> DataLoader:
    """,
...
def train_unet(
    ...
    for epoch in range(num_epochs):
        ...
        if logger and iou:
            logger.info(
                f"Epoch {epoch + 1}/{num_epochs}, Train Loss: {average_train_loss:.4f}, IOU Score: {iou_score_mean:.4f}"
            )
        elif logger is not None:
            logger.info(
                f"Epoch {epoch + 1}/{num_epochs}, Train Loss: {average_train_loss:.4f}"
            )
        ...
        with torch.no_grad():
            for inputs, targets in tqdm(
                test_data_loader, desc=f"Validation", leave=False
            ):
                outputs = model(inputs)
                targets = targets.squeeze(1)
                outputs.to(device)
        ...
        if logger and iou:
            logger.info(
                f"Epoch {epoch + 1}/{num_epochs}, Validation Loss: {average_val_loss:.4f}. IOU Score: {iou_score_mean:.4f}"
            )
        elif logger is not None:
            logger.info(
