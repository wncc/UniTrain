    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (inputs, labels) in enumerate(loop):
            ...
            if batch_idx % 100 == 99:  # Print and log every 100 batches
                ...
                wnb_path = os.path.join(
                    wnb_dir, f"model_epoch_{epoch + 1}_batch{batch_idx + 1}.pth"
                )
                torch.save(model.state_dict(), wnb_path)
                if logger:
                    logger.info(
                        f"Epoch {epoch + 1}, Batch {batch_idx + 1}, Loss: {avg_loss:.4f}, wnbPath: {wnb_path}"
                    )
        ...
        if logger:
            logger.info(
                f"Epoch {epoch + 1}, Validation Accuracy: {accuracy:.2f}%, wnbPath: {wnb_path}"
            )
